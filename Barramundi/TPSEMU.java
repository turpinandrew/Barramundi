...
public class TPSEMU extends AbstractTestProcedure {
                                                   int sp;
                                                   int ap;
    ...
    TPSEMU(Double sp, Double ap, String pdfFilename, String lfFilename, String stopCondition, Double stopP)
    {
        super("SEMU sp= "+ sp+ " ap= "+ap+" "+pdfFilename + " " + lfFilename+" "+stopCondition+" "+stopP);
        ...
        if (stopCondition.compareTo("NUMBER_OF_PRESENTATIONS") == 0)
            ...
            JOptionPane.showMessageDialog(null,"The stopping condition "+stopCondition+" for TPSEMU is unknown","Barramundi - Error",JOptionPane.ERROR_MESSAGE);
    ...
    public void run(Patient p) {
        ...
        for (int i = 0; i < points.size(); i++) {
         ...
         stats.numPresentation[loc]++;
            numPresentations++;
            boolean spSeen = p.seenObject.seen(sp, p.threshold[loc], numPresentations, point.getX(), point.getY());
            boolean apSeen = false;
            if (spSeen) {
             stats.numPresentation[loc]++;
                numPresentations++;
                apSeen = p.seenObject.seen(ap, p.threshold[loc], numPresentations, point.getX(), point.getY());
            }
            if (!spSeen || !apSeen) {
                ...
                stats.measuredThreshold[loc] = Math.max(sp,ap);
            }
        }
    }
