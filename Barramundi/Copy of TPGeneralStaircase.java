...
class TPGeneralStaircase extends AbstractTestProcedure {
    ...
    */
    TPGeneralStaircase(Double numUp, Double numDown, Double numReversals, 
					   Double numLastReversals, Double startGuess) {
    ...
    public void run(Patient p) {
		...
		*******************************************************/
		...
		for (int i = 0; i < points.size(); i++)
		{
		    ...
		    /*******************************************************
            MAY NEED MORE PARAMS HERE:
		    *******************************************************/
			results.add(new Stair(x, y, number, inputStartGuess));   
		...
		try 
		{
            ...
            while (results.size() > 0)
            {
                ...
                if (stair.numPresentations > 1)
                {
					...
					changed!
					***************************************************/
					else if (stair.numTimesMissedZero > 3)
					{
                        stair.done = true;
						finishedResults.add(results.remove(loc));
                		stats.measuredThreshold[stair.locationNumber] = 0;
                    }
					else if (stair.numTimesSeenForty > 3)
					{
                        stair.done = true;
						finishedResults.add(results.remove(loc));
